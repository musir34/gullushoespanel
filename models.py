...
# Kar analizi verilerini saklama tablosu
class ProfitData(db.Model):
    """Kar analizi verilerini saklayan tablo"""
    __tablename__ = 'profit_data'

    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.Date, nullable=False)
    data_type = db.Column(db.String(50), nullable=False)  # 'daily', 'product', 'model', 'color', 'size'
    reference_id = db.Column(db.String(100), nullable=True)  # ürün barkodu, model kodu vs.
    reference_name = db.Column(db.String(255), nullable=True)  # ürün/model/renk/beden adı
    ...
    # Bileşik indeks, aynı gün için aynı veri tipinin tekrarlanmaması için
    __table_args__ = (
        db.UniqueConstraint('date', 'data_type', 'reference_id', name='uix_profit_data'),
    )

    def __repr__(self):
        return f"<ProfitData {self.date} {self.data_type} {self.reference_id}>"

# Ürün maliyet bilgilerini saklamak için tablo
class ProductCost(db.Model):
    """Ürün maliyet bilgilerini saklayan tablo"""
    __tablename__ = 'product_costs'
    
    barcode = db.Column(db.String(100), primary_key=True)
    cost_price = db.Column(db.Float, nullable=False)
    source = db.Column(db.String(50))  # 'siparis_fisi', 'manual', 'calculated'
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    def __repr__(self):
        return f"<ProductCost {self.barcode} {self.cost_price}>"